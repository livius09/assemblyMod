            ...
            for (ItemEntity item : items) {
                ...
                if (stack.getItem() instanceof BlockItem blockItem) {
                    ...
                    if (blockAtTarget == Blocks.AIR
                            || blockAtTarget == Blocks.GRASS_BLOCK
                            || blockAtTarget == Blocks.TALL_GRASS
                            || blockAtTarget == Blocks.FERN
                            || blockAtTarget == Blocks.DEAD_BUSH
                            || blockAtTarget == Blocks.SNOW) {
                            world.setBlockState(targetPos, blockItem.getBlock().getDefaultState());
                            stack.decrement(1);

                            if (stack.isEmpty()) {
                                item.discard(); // remove entity if no items left
                            }
                        }
                    }
